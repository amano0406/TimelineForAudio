...
from timeline_for_audio_worker.runtime_guard import (
    ...
    is_host_cli_allowed_for_tests,
    is_running_in_container,
)


class RuntimeGuardTests(unittest.TestCase):
    def test_host_cli_is_blocked_without_explicit_test_override(self) -> None:
        with (
    ...
    def test_host_cli_is_allowed_with_explicit_test_override(self) -> None:
        with (
            patch.dict(os.environ, {ALLOW_HOST_CLI_ENV: "1"}, clear=True),
            patch(
                "timeline_for_audio_worker.runtime_guard._is_docker_marker_present",
                return_value=False,
            ),
        ):
            self.assertTrue(is_host_cli_allowed_for_tests())
            assert_cli_runtime_allowed()
