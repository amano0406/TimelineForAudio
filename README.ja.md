...
2. 音声を文字起こししてタイムスタンプ付き transcript を作ります
3. 必要に応じて話者分離を適用します
4. 設定されていれば deterministic な transcript normalization を適用します
5. pause、loudness、speaking rate、pitch、overlap などの音声要約を計算します
6. 最終結果を ZIP にまとめます
...
- `timelines/<収録日時>.md`
- `raw-transcripts/<収録日時>.md`
- `normalized-transcripts/<収録日時>.md`
- `normalization-reports/<収録日時>.md`
...
`README.html` が export の入口です。そこから timeline、transcript、normalization report、speaker summary、audio feature summary を辿れます。
...
- worker ログ
- 正規化済み音声や probe 情報
- raw / normalized transcript の JSON と markdown
