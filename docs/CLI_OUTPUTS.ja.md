...
Windows から呼び出す場合は、正面入口として `cli.bat` を使います。`cli.ps1` は `cli.bat` から呼ばれる内部実装です。
...
```powershell
.\cli.bat settings validate-token --json
.\cli.bat settings validate-token --token hf_xxx --json
```
...
```powershell
.\cli.bat settings save --token hf_xxx --compute-mode gpu --json
```
...
```powershell
.\cli.bat settings inputs list --json
.\cli.bat settings inputs add "C:\TimelineData\input-audio\" --json
.\cli.bat settings inputs remove "C:\TimelineData\input-audio\" --json
.\cli.bat settings inputs clear --json
```
...
| `remove` | 削除後の入力元一覧 |
| `remove` で存在しない path | エラーにせず、既存一覧をそのまま返す |
| `clear` | 空配列 |
...
```powershell
.\cli.bat settings master show --json
.\cli.bat settings master set "C:\TimelineData\audio" --json
```

成功時:
マスター保存先の path 文字列を返します。

```json
"C:\\TimelineData\\audio"
```
...
.\cli.bat files list --probe --page 1 --page-size 50 --json
```
...
```json
{
  ...
  "files": [
    {
      "source_id": "C:\\TimelineData\\input-audio\\",
      "source_display_name": "C:\\TimelineData\\input-audio\\",
      "root_path": "C:\\TimelineData\\input-audio\\",
      "relative_path": "meeting/sample.wav",
      "directory": "meeting",
      "file_name": "sample.wav",
      "display_path": "C:\\TimelineData\\input-audio\\meeting\\sample.wav",
      "container_path": "/host/input/input-1/meeting/sample.wav",
      "size_bytes": 123456,
      ...
      "turn_count": 8,
      "speaker_count": 2,
      "source_file_identity": "C:\\TimelineData\\input-audio\\::meeting/sample.wav"
    }
  ]
}
```
...
| パターン | 返却 |
|---|---|
| 対象ファイルなし | `files=[]`, `pagination.total_files=0` |
...
```json
{
  ...
  "audio_files": [
    {
      "source_name": "C:\\TimelineData\\input-audio\\",
      "path": "/host/input/input-1/sample.wav",
      "size_bytes": 123456
    }
  ]
}
  ...
  "items": [
    {
      ...
      "updated_at": "2026-04-30T10:00:10+00:00",
      "status": "available",
...
.\cli.bat items refresh --queue-only --json
.\cli.bat items refresh --input-root "C:\TimelineData\input-audio\" --json
```
  ...
  "skipped": [
    {
      ...
      "source_file_identity": "C:\\TimelineData\\input-audio\\::sample.wav",
      "reason": "unchanged",
      "source_hash": "sha256...",
...
```json
{
  "state": "pending",
  "run_id": "run-20260430-100000-abcdef12",
  "run_dir": "/shared/app-data/<master-key>/runs/run-20260430-100000-abcdef12",
  "artifact": "timeline",
  "queue_only": true,
  "queued_count": 3,
  "skipped_count": 0,
...
| `--input-root` | 指定した入力ディレクトリだけ refresh する。複数回指定可能 |
...
```powershell
.\cli.bat items remove --item-id item-a,item-b --dry-run --json
.\cli.bat items remove --item-id item-a,item-b --json
```
...
.\cli.bat items download --output "C:\Temp\all-items.zip" --json
.\cli.bat items download --item-id item-a,item-b --json
.\cli.bat items download --item-id item-a,item-b --output "C:\Temp\items.zip" --json
```

`--item-id` を指定しない場合は、利用可能な全 item を ZIP に含めます。
...
| `--item-id` なし | `status=available` の全 item を ZIP に含める |
| 全件対象で利用可能 item がない | 非 0。`At least one available item id is required.` |
...
```powershell
.\cli.bat runs show --run-id run-20260430-100000-abcdef12 --json
```
