...
Windows から呼び出す場合は、正面入口として `cli.ps1` を使います。`cli.bat` は PowerShell script を直接起動できない環境向けの互換ラッパーです。
...
| `--json` 付きの実行時エラー | JSON error envelope | 非 0 |
| `--json` なしの実行時エラー | 人間向けエラーテキスト | 非 0 |
...
- `--json` 付きの実行時エラーは、可能な範囲で次の JSON error envelope を stdout に返します。管理 UI 側は `exit code != 0` を失敗として扱い、stdout に JSON があれば `error.message` を表示してください。
- Docker worker を起動できない場合も、`--json` 付きなら wrapper が `DockerWorkerInvocationError` の JSON error envelope を stdout に返します。
...
| 設定が壊れている | `--json` 付きなら JSON error envelope。`--json` なしなら人間向けエラーテキスト |
| Docker が起動していない | 非 0。Docker / compose 側のエラーテキスト |
| path が存在しない | コマンドにより異なる。入力ファイル実処理では非 0、一覧系では空または `missing_sources` |
| Hugging Face API へ接続できない | `models list --include-remote` は model 行の `huggingface.remote_status=error` |
...
.\cli.ps1 files list --probe --page 1 --page-size 50 --json
```
...
.\cli.ps1 items list --page 2 --page-size 50 --json
```
...
.\cli.ps1 items refresh --input-root "C:\TimelineData\input-audio\" --json
```
...
1. すべて `--json` 付きで呼ぶ。
2. `exit code != 0` はコマンド失敗として扱う。stdout に JSON がある場合は `error.message` を主表示にし、JSON がない場合だけ stdout/stderr のテキストを詳細に出す。
3. 一覧系コマンドは空配列を正常な空状態として扱う。
4. `items remove` は `missing_item_ids` があってもコマンド成功として扱い、削除できた件数と見つからなかった ID を分けて表示する。
5. `items refresh` は `state` と `queued_count` を主判定にし、`skipped` / `deferred` は詳細表示に回す。
